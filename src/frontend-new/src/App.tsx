...
const DifficultyDots: React.FC<{ level: 1 | 2 | 3; color: string }> = ({ level, color }) => (
...
const FeaturedCard: React.FC<{ rec: IRecommendation; onStart: (rec: IRecommendation) => void }> = ({ rec, onStart }) => {
    const c = COLOR_MAP[rec.color];
    const btnClass = rec.color === 'orange' ? 'bg-orange-500 hover:bg-orange-400'
        : rec.color === 'blue' ? 'bg-blue-600 hover:bg-blue-500'
            : rec.color === 'emerald' ? 'bg-emerald-600 hover:bg-emerald-500'
        ...
        <div className={`relative rounded-2xl border ${c.border} ${c.bg} p-6 overflow-hidden col-span-2`}>
            ...
            <div className="flex items-start justify-between relative z-10">
                <div>
                    ...
                    <button onClick={() => onStart(rec)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all hover:scale-105 active:scale-95 text-white ${btnClass}`}>
...
const TrainingCard: React.FC<{ rec: IRecommendation; onStart: (rec: IRecommendation) => void }> = ({ rec, onStart }) => {
    const c = COLOR_MAP[rec.color];
    return (
        <div className={`group relative rounded-2xl border ${c.border} bg-slate-900/60 p-5 flex flex-col gap-4 cursor-pointer hover:${c.bg} transition-all duration-300`}
             onClick={() => onStart(rec)}>
...
const App: React.FC = () => {
    ...
    const handleStartTraining = async (rec: IRecommendation) => {
        setLoadingId(rec.id);
    ...
    const filtered = RECOMMENDATIONS.filter(r => !r.featured && (activeFilter === 'Все' || r.category === activeFilter));
                ...
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1.5 text-sm text-slate-400">
                            ...
                            <span className="font-bold text-white">8</span> тренировок
                        </div>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-5xl mx-auto px-6 py-8">
                {message && (
                    <div className={`mb-6 flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium ${messageType === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-rose-500/10 border-rose-500/30 text-rose-300'}`}>
                        <Zap size={16} />
                        {message}
                        <button onClick={() => setMessage('')} className="ml-auto opacity-50 hover:opacity-100 transition text-lg leading-none">×</button>
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {filtered.map(rec => (
                        <div key={rec.id} className="relative">
                            ...
                            <TrainingCard rec={rec} onStart={handleStartTraining} />
                        </div>
                    ))}
                </div>
            </main>
